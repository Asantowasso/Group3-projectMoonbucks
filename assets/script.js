...
var currencyOptions = ["GBP", "EUR", "USD"];
var currencyInput = currencyOptions[1];
...
function changeCurrency(event) {
  ...
  if (document.getElementById("USD").checked) {
    currencyInput = currencyOptions[2];
  } else if (document.getElementById("GBP").checked) {
    currencyInput = currencyOptions[0];
  } else {
    currencyInput = currencyOptions[1];
  }
  convertCurrency(currencyInput);
  getMenu();
}

function convertCurrency(currencyInput) {
...
function displayMenu(data) {
  //iterates through the first 10 items in the array fetched from the API and creates and appends an element for each
    ...
    var child = menuEl.lastElementChild; 
    while (child) {
        menuEl.removeChild(child);
        child = menuEl.lastElementChild;
    }
...
currencyForm.addEventListener("submit", changeCurrency);
